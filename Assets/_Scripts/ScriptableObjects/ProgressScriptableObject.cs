    ...
    public class ProgressScriptableObject : ScriptableObject//, IXmlSerializable
    {
        ...
        public int CountOfLevelsInCurrentBatch
        {
            ...
            set
            {
                ...
                {
                    prog.LevelIndicesToSelectFromCurrentBatch.RemoveAt(
                        prog.LevelIndicesToSelectFromCurrentBatch.Count - 1);
                }
        ...
        void OnEnable()
        {
            try
            {
                prog = MyXMLSerializer.Deserialize<Progress>(
                    Application.persistentDataPath + Constants.ProgressFileName);
            }
            catch
            {
                MyXMLSerializer.Serialize<Progress>(
                    Application.persistentDataPath + Constants.ProgressFileName, prog);
